...
public class policy {
	...
	public void createOntology(OntModel m) {
		...
		if (LocalAddress.contains("/"))// 有掩码
		{
			...
			queryString = wfp + "SELECT ?x WHERE { ?x wfp:networkNumberIs \"" + netNumber + "\".?x wfp:maskIs \""
					+ netmask + "\"}";// 有问题
			Query query = QueryFactory.create(queryString);
			QueryExecution qe = QueryExecutionFactory.create(query, m);
			ResultSet results = qe.execSelect();
		 ...
		 * process1=process.createIndividual(NS+"Process"+P++);// 因为每新生成一个新的网络节点，都要加1.
		...
		if (!this.程序.equals("任何")) {
			...
			if (host1 == null) {
				...
				queryString = wfp + " SELECT ?x WHERE { ?x wfp:programPathIs ?y" + ".?x wfp:runningOn wfp:Host" + H
						+ "\r\n" + ".FILTER regex(?y,\"" + this.程序 + "\")}";
				Query query = QueryFactory.create(queryString);
				QueryExecution qe = QueryExecutionFactory.create(query, m);
				ResultSet results = qe.execSelect();
		...
		if (!this.本地端口.equals("任何")) {
			OntClass process = m.getOntClass(NS + "Process");
			ObjectProperty obp1 = m.getObjectProperty(NS + "runningOn");
			DatatypeProperty dtpPort = m.getDatatypeProperty(NS + "portIs");
			if (host1 == null) {
				queryString = wfp + " SELECT ?x WHERE { ?x wfp:portIs ?y .?x wfp:runningOn wfp:AllEntities \r\n"
						+ ".FILTER regex(?y,\"" + this.本地端口 + "\")}";
				Query query = QueryFactory.create(queryString);
				QueryExecution qe = QueryExecutionFactory.create(query, m);
				ResultSet results = qe.execSelect();
				if (!results.hasNext()) {
			...
			if (!results.hasNext()) {
				...
				if (!this.程序.equals("任何")) {
					if (process1 == null) {
						...
						m.add(process1, obpObject, access1);
					if (host1 == null)
					...
					if (process1 == null) {
						Query query1 = QueryFactory.create(wfp + "SELECT ?x WHERE{?x wfp:portIs ?y \r\n"
								+ ".FILTER regex(?y,\"" + this.本地端口 + "\")}");
						QueryExecution qe1 = QueryExecutionFactory.create(query1, m);
						ResultSet results1 = qe1.execSelect();
						QuerySolution QS = results1.nextSolution();
		...
		if (this.远程地址.contains("/") && (!this.远程地址.equals("任何")) && (this.远程端口.equals("任何"))) {
			...
			/*
			 * if ((!this.远程端口.equals("任何"))) { m.add(process2, obp111, rule1); }
			 */
		}
		...
		if (this.操作 == "安全连接") {
			...
			if (!results.hasNext()) {
				/*
				 * OntClass Control = m.getOntClass(
				 * "http://www.semanticweb.org/administrator/ontologies/2018/3/untitled-ontology-27#"
				 * +op); control1 = Control.createIndividual(NS + op + ++C);
				 */
